...
function dedupeNumberedSections(markdown) {
  ...
  for (const line of lines) {
    const heading = /^#+\s+([1-5])\.\s+/.exec(line);
    if (heading) {
      if (seen.has(heading[1])) { skipping = true; continue; }
      seen.add(heading[1]);
      skipping = false;
    }
...
function buildFieldValues(markdown) {
  ...
  return {
    분석개요: sectionBody(md, /^#+\s*1\. 분석 개요/m, [/^#+\s*2\. Agent 진단 결과/m]),
    ...
    policy: sectionBody(md, /^#+\s+.*Policy Agent\b/m, [/^#+\s+3\. Agent 검증/m]),
    challenger: sectionBody(md, /^#+\s+.*Challenger Agent\b/m, [/^#+\s+4\. Agent 종합\s*분석/m]),
    transition: sectionBody(md, /^#+\s+.*Transition Agent\b/m, [/^#+\s+5\. NEXT 전환경로 설계/m]),
...
module.exports = { TEMPLATE_PATH, buildTemplateReport, buildFieldValues, normalizeClickHereNames, toReportText, dedupeNumberedSections };
