...
async function normalizeClickHereNames(input, projectName = "") {
  ...
  for (const name of sectionNames) {
    ...
    // 과제명(과제번호).
    xml = xml.replace(/<hp:t>[^<]*과제명\s*-<\/hp:t>/g, `<hp:t>${safeProjectName}</hp:t>`);
    zip.file(name, xml);
  }
...
module.exports = { TEMPLATE_PATH, buildTemplateReport, buildFieldValues, normalizeClickHereNames, toReportText };
