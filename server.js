...
async function handleHwpxReport(request, response) {
  try {
    ...
    // Use Kordoc's official-report layout so Hancom does not have to infer
    // ad-hoc paragraph styles from the raw Markdown output.
    const hwpx = await markdownToHwpx(markdown, {
      gongmun: {
        preset: "보고서",
        // The supplied reference form has no running header/cover banner.
        // Keep the task title in the Markdown body instead of adding a
        // generated cover header that can burden Hancom's layout engine.
        cover: false,
        toc: false,
        pageNumbers: true,
        endMark: false,
