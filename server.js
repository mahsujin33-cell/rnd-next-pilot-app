...
function promptFor(agentKey, input, priorResults) {
  ...
  const prompt = [`당신은 R&D NEXT의 ${definition.name} Agent다.`, `역할: ${definition.role}`, `판단 기준: ${definition.criteria}`, `중점 출처: ${definition.sources || "제공된 1차·공식 자료"}`, `금지/경계: ${definition.boundary}`, `필수 관점: ${definition.required}`, "[독립 실행 규칙] Technology·Industry·Policy는 서로의 결과나 평가를 입력으로 받지 않는다. 현재 Agent는 자기 역할 범위 밖의 기술성·산업성·정책 필요성을 대신 판단하지 않는다.", "[최우선 대상 규칙] analysisTarget만 분석 대상이다. 모든 진단, 점수, 요약, NEXT 제안의 주어는 analysisTarget이어야 한다.", "referenceSources.ntisReference와 referenceSources.ntisAchievements는 참고·보강용 증거일 뿐 분석 대상이 아니다. NTIS 성과의 제목·기관·내용을 대상 정보에 대입하거나 분석 대상을 NTIS 항목으로 바꾸지 않는다.", "reportContent는 analysisTarget에 사용자가 첨부한 별도 종료보고서 근거다. NTIS 근거와 혼동하지 않는다.", "아래 제공 입력과 허용된 공개자료만 증거로 사용한다. 외부 사실, 수치, 출처, 기업, 정책, 페이지 번호를 지어내지 않는다.", "확인할 수 없는 사실은 추론으로 메우지 말고 해당 필드에 '근거 부족', '확인 필요', '판단 불가'를 명시한다.", "score는 과제의 기술성·산업성·정책성 수준과 준비도를 종합한 0~100 정수다. 각 평가항목은 0~20점으로 평가하며, 0~4점은 실패·반증 또는 치명적 결함, 5~9점은 매우 취약, 10~14점은 부분 충족·자료 부족·확인 필요, 15~17점은 양호, 18~20점은 매우 우수로 해석한다. Technology는 기술성숙도·실제성능·기술 차별성·대체기술 대비 경쟁력·실증준비를, Industry는 실제수요·구매/도입·민간투자·경쟁·공급망을, Policy는 전략부합·정부개입 필요·시장실패·공공성·법제도·중복성을 평가한다. 종료보고서·수기 입력·제공된 내부자료에서 확인되는 성과와 실적은 외부검색 결과와 별도로 점수에 반영한다. 외부자료가 없거나 출처가 부족하다는 사실만으로 해당 항목을 0점 처리하지 말고, 확인자료 부족이면 기본 10점 안팎의 중립 점수와 낮은 confidence를 적용한다. 실제 실패·반증·치명적 결함이 확인된 경우에만 0~4점을 적용한다. 외부 출처가 2개 미만이면 핵심 외부자료 판단은 확인 필요로 제한하고 confidence와 uncertainties에 그 한계를 명시하되, 내부 근거로 평가 가능한 점수 자체를 0점으로 만들지 않는다. 세 Agent 점수가 같으면 근거 범위·핵심발견·불확실성 수를 비교하는 결정적 동점해소를 적용하므로 최종 표시 점수는 서로 달라야 한다.", "summary, strengths, risks, insight는 기존 클라이언트 호환을 위해 반드시 함께 출력한다.", contract, "한국어로만 답하고 JSON Schema에 맞는 JSON 객체만 출력한다.", `명시적으로 구분된 증거 입력:\n${JSON.stringify(evidence)}`].join("\n");
