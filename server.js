...
const HWP_PARSER_TIMEOUT_MS = 300000;
...
function normalizeInput(body) {
  ...
  return { apiKey, agents, priorResults, analysisInput: { targetProject, sources: { ntisReference, ntisAchievements }, externalEvidence: {}, reportContent } };
}
...
function compactAgentResult(agentKey, result, tight = false) {
  ...
  if (["technology", "industry", "policy"].includes(agentKey)) {
    ...
    compact.searchStatus = cleanString(result.searchStatus, 40);
    compact.sourceCount = Number.isFinite(result.sourceCount) ? result.sourceCount : 0;
    compact.externalEvidenceStatus = cleanString(result.externalEvidenceStatus, 40);
    compact.externalSources = Array.isArray(result.externalSources) ? result.externalSources.slice(0, tight ? 2 : 6) : [];
...
function compactPromptInput(input, agentKey) {
  ...
  return {
    ...
    externalEvidence: input.analysisInput.externalEvidence?.[agentKey] || { status: "unavailable", sourceCount: 0, sources: [], searchQueries: [] },
    reportContent: report ? { ...report, warnings: capList(report.warnings, 3, 180), content: cleanString(report.content, reportLimit) } : null
  };
}

function searchRoleContract(agentKey) {
  return agentKey === "technology"
    ? "표준·논문·특허·경쟁기술·기업 기술개발·기술성숙도·상용화 사례"
    : agentKey === "industry"
      ? "기업·제품·구매·도입·시장·투자·경쟁사·밸류체인·국내외 수요"
      : "정부정책·법령·규제·표준·인증·국가R&D 사업·기술로드맵·해외 정책";
}
...
function extractResponseText(payload) {
  ...
  for (const item of Array.isArray(payload?.output) ? payload.output : []) {
    for (const content of Array.isArray(item?.content) ? item.content : []) if (typeof content?.text === "string") chunks.push(content.text);
  }
  return chunks.join("\n");
}

function collectWebSources(payload, agentKey) {
  const sources = [];
  const visit = (value) => {
    if (!value || typeof value !== "object") return;
    if (typeof value.url === "string" && /^https?:\/\//i.test(value.url) && (value.title || value.name || value.publisher)) {
      sources.push({ title: cleanString(value.title || value.name, 300) || "제목 확인 필요", url: value.url.slice(0, 1000), publisher: cleanString(value.publisher || value.source, 200) || "확인 필요", publishedDate: cleanString(value.publishedDate || value.date, 40) || "확인되지 않음", accessedAt: new Date().toISOString(), agent: agentKey, reliability: "외부 검색 citation", supportingClaim: "검색 결과 원문 확인 필요" });
    }
...
async function collectExternalEvidence(apiKey, agentKey, input) {
  const target = input.analysisInput.targetProject;
  const querySeed = cleanString(target.name, 200);
  const queries = [
    `${querySeed} ${agentKey === "technology" ? "standard paper patent competing technology" : agentKey === "industry" ? "companies products adoption market investment" : "government policy law regulation standard R&D"}`,
    `${querySeed} ${searchRoleContract(agentKey)} recent`
  ];
  const requestId = input.diagnosticId;
  const startedAt = Date.now();
  const prompt = `R&D ${agentKey} 외부자료 수집 담당자다. 대상 과제는 ${querySeed}다. 실제 웹 검색을 수행해라. 검색 범위: ${searchRoleContract(agentKey)}. 다음 검색어를 각각 활용하고 필요하면 변형 검색어를 추가해라: ${queries.join(" / ")}. 종료보고서 사실과 외부자료를 구분한다. 검색 결과가 없는 자료는 만들지 않는다. JSON으로만 반환한다: {"searchQueries":[string],"claims":[string],"sources":[{"title":string,"url":string,"publisher":string,"publishedDate":string,"reliability":string,"supportingClaim":string}]}. URL은 실제 검색 citation에서만 복사하고, 날짜를 확인하지 못하면 '확인되지 않음'으로 쓴다.`;
  const controller = new AbortController();
  const timer = setTimeout(() => controller.abort(), 90000);
  try {
    let response = await fetch("https://api.openai.com/v1/responses", { method: "POST", headers: { Authorization: `Bearer ${apiKey}`, "Content-Type": "application/json" }, body: JSON.stringify({ model: OPENAI_SEARCH_MODEL, tools: [{ type: "web_search", external_web_access: true, search_context_size: "medium" }], tool_choice: "required", include: ["web_search_call.action.sources"], input: prompt }), signal: controller.signal });
    let payload = await response.json().catch(() => ({}));
    if (!response.ok && [400, 404].includes(response.status)) {
      const previewResponse = await fetch("https://api.openai.com/v1/responses", { method: "POST", headers: { Authorization: `Bearer ${apiKey}`, "Content-Type": "application/json" }, body: JSON.stringify({ model: OPENAI_SEARCH_MODEL, tools: [{ type: "web_search_preview" }], tool_choice: "required", input: prompt }), signal: controller.signal });
      const previewPayload = await previewResponse.json().catch(() => ({}));
      if (previewResponse.ok) { response = previewResponse; payload = previewPayload; }
    }
    if (!response.ok) { const error = new Error(safeProviderMessage(response.status)); error.providerStatus = response.status; error.statusCode = response.status === 401 ? 401 : response.status === 429 ? 429 : 502; error.code = `OPENAI_SEARCH_${response.status}`; throw error; }
    const generated = parseSearchJson(extractResponseText(payload));
    const sources = collectWebSources(payload, agentKey);
    let uniqueSources = [...new Map(sources.map((item) => [item.url, { title: cleanString(item.title, 300) || "제목 확인 필요", url: item.url, publisher: cleanString(item.publisher, 200) || "확인 필요", publishedDate: cleanString(item.publishedDate, 40) || "확인되지 않음", accessedAt: new Date().toISOString(), agent: agentKey, reliability: cleanString(item.reliability, 120) || "외부 검색 citation", supportingClaim: cleanString(item.supportingClaim, 500) || "확인 필요" }])).values()];
    if (uniqueSources.length < 2) {
      const retryQuery = queries.find((query) => query !== queries[0]) || `${querySeed} ${searchRoleContract(agentKey)} official source`;
      const retryPrompt = `R&D ${agentKey} 외부자료 추가 검증이다. 반드시 실제 웹 검색을 실행하고, 첫 검색과 다른 공식·신뢰 출처 URL을 최소 2개 찾아라. 검색어: ${retryQuery}. URL은 실제 web_search citation에서만 복사한다. JSON으로만 반환한다: {"claims":[string]}`;
      const retryResponse = await fetch("https://api.openai.com/v1/responses", { method: "POST", headers: { Authorization: `Bearer ${apiKey}`, "Content-Type": "application/json" }, body: JSON.stringify({ model: OPENAI_SEARCH_MODEL, tools: [{ type: "web_search_preview" }], tool_choice: "required", input: retryPrompt }), signal: controller.signal });
      const retryPayload = await retryResponse.json().catch(() => ({}));
      if (retryResponse.ok) {
        const retrySources = collectWebSources(retryPayload, agentKey).map((item) => ({ title: cleanString(item.title, 300) || "제목 확인 필요", url: item.url, publisher: cleanString(item.publisher, 200) || "확인 필요", publishedDate: cleanString(item.publishedDate, 40) || "확인되지 않음", accessedAt: new Date().toISOString(), agent: agentKey, reliability: "외부 검색 citation", supportingClaim: "추가 검색 결과 원문 확인 필요" }));
        uniqueSources = [...new Map([...uniqueSources, ...retrySources].map((item) => [item.url, item])).values()].slice(0, 12);
      }
    }
    return { status: uniqueSources.length >= 2 ? "success" : "insufficient_sources", sourceCount: uniqueSources.length, minimumSourceCount: 2, sources: uniqueSources, claims: Array.isArray(generated.claims) ? generated.claims.slice(0, 8) : [], searchQueries: Array.isArray(generated.searchQueries) && generated.searchQueries.length ? generated.searchQueries.slice(0, 6) : queries, searchedAt: new Date().toISOString() };
  } catch (error) {
    diagnosticLog("external.search.error", { requestId, agent: agentKey, elapsedMs: Date.now() - startedAt, error: diagnosticError(error) });
    return { status: "failed", sourceCount: 0, minimumSourceCount: 2, sources: [], claims: [], searchQueries: queries, searchedAt: new Date().toISOString(), error: error.code || "OPENAI_SEARCH_FAILED" };
  } finally { clearTimeout(timer); }
}
...
function promptFor(agentKey, input, priorResults) {
  ...
  const domainContract = "Technology/Industry/Policy는 서로의 결과를 보지 않고 analysisTarget·종료보고서·NTIS 참고성과와 자기 Agent에 제공된 External Evidence만으로 독립 수행한다. External Evidence는 실제 Responses API web_search citation에서 수집된 자료다. 외부자료를 조사했다고 주장하려면 제공된 External Evidence의 URL을 반드시 연결하고, URL·논문·특허·기업·정책을 임의로 만들지 않는다. 외부 출처가 2개 미만이거나 status가 success가 아니면 외부근거 부족으로 표시하고 시장성·정책성·경쟁력을 확정하지 않는다. 핵심 주장은 서로 다른 출처 2개 이상으로 교차검증한다. 모든 핵심 evidence에는 claim, source, publishedDate, url, sourceGrade(A/B/C/D), certainty, sourceType, pageOrId를 남긴다. 출처 우선순위는 정부·공공·표준기관·공식 기업자료·원 논문·특허(A) > 전문연구기관·학술DB·증권사 리서치(B) > 시장조사기관·전문매체(C) > 일반뉴스·블로그(D)이며 증권사 자료는 단독 핵심근거로 쓰지 않는다. 근거가 부족하면 추정하지 말고 '근거 부족', '확인 필요', '판단 불가'로 표시한다. conclusion(판정), keyFindings 3~6개, evidence 3~6개, uncertainties 3~5개, bottlenecks 2~4개, nextCandidates 2~4개, dataRequests 2~5개, confidenceRationale을 구체적으로 작성한다.";
...
function normalizeAgentResult(agentKey, value) {
  ...
  if (["technology", "industry", "policy"].includes(agentKey)) {
    ...
    result.evidence = Array.isArray(value?.evidence) ? value.evidence.slice(0, 6).map((item) => ({
      claim: cleanString(item?.claim, 800), source: cleanString(item?.source, 300) || "확인 불가", publishedDate: cleanString(item?.publishedDate, 40) || "확인되지 않음", url: cleanString(item?.url, 800), sourceGrade: ["A", "B", "C", "D", "확인 불가"].includes(item?.sourceGrade) ? item.sourceGrade : "확인 불가",
    ...
    const severeFailureText = [result.summary, result.conclusion, ...result.risks, ...result.bottlenecks].join(" ");
    const hasExplicitFailure = /(실패|중단|치명적|달성하지 못|전혀 없음|불가능)/u.test(severeFailureText);
    const hasSubstantialEvidence = result.keyFindings.length >= 3 || result.evidence.length >= 3 || result.strengths.length >= 2;
    if (result.score < 20 && hasSubstantialEvidence && !hasExplicitFailure) result.score = 35;
    ...
    result.confidence = Number.isFinite(rawConfidence) ? Math.max(0, Math.min(100, Math.round(rawConfidence))) : score;
    result.confidenceRationale = cleanString(value?.confidenceRationale, 1200) || "추가 판단 필요";
...
async function callOpenAI(apiKey, agentKey, input, priorResults) {
  ...
  const result = normalizeAgentResult(agentKey, parsed);
  if (["technology", "industry", "policy"].includes(agentKey)) {
    const external = input.analysisInput.externalEvidence?.[agentKey] || { status: "unavailable", sourceCount: 0, minimumSourceCount: 2, sources: [], searchQueries: [] };
    result.searchStatus = external.status;
    result.searchError = cleanString(external.error, 160) || "";
    result.searchQueries = external.searchQueries || [];
    result.externalSources = external.sources || [];
    result.sourceCount = external.sourceCount || 0;
    result.minimumSourceCount = external.minimumSourceCount || 2;
    const allowedUrls = new Set(result.externalSources.map((source) => source.url).filter(Boolean));
    result.evidence = result.evidence.map((item) => item.url && !allowedUrls.has(item.url) ? { ...item, url: "", sourceGrade: "확인 불가", certainty: "확인 불가" } : item);
    if (external.status !== "success" || result.sourceCount < 2) {
      result.externalEvidenceStatus = "insufficient_sources";
      result.conclusion = `외부근거 부족(${result.sourceCount}/${result.minimumSourceCount}개)으로 외부자료 기반 판단은 확인 필요: ${result.conclusion}`;
      result.uncertainties = [...new Set(["외부자료 출처 2개 미만 또는 검색 실패", ...result.uncertainties])].slice(0, 5);
    } else result.externalEvidenceStatus = "sufficient";
  }
  return result;
}
...
async function handleAgentRun(request, response) {
  ...
  try {
    ...
    if (firstWave.length) {
      const externalResults = await Promise.all(firstWave.map(async (agent) => {
        if (stream) emit({ type: "progress", phase: "external-search", agent, status: "running", message: `${AGENT_DEFINITIONS[agent].name} 외부자료를 검색하는 중입니다.` });
        const evidence = await collectExternalEvidence(input.apiKey, agent, input);
        input.analysisInput.externalEvidence[agent] = evidence;
        if (stream) emit({ type: "external-search", phase: "external-search", agent, status: evidence.status, sourceCount: evidence.sourceCount, minimumSourceCount: evidence.minimumSourceCount, sources: evidence.sources, searchQueries: evidence.searchQueries });
        return evidence;
      }));
      if (stream) emit({ type: "progress", phase: "external-search", status: "completed", completedAgents: firstWave.filter((agent, index) => externalResults[index]?.status === "success") });
    }
...
function runKordoc(inputPath, requestId, extension) {
  const startedAt = Date.now();
  const timeoutMs = extension === "hwp" ? HWP_PARSER_TIMEOUT_MS : PARSER_TIMEOUT_MS;
  diagnosticLog("parser.request.start", { requestId, extension, parser: "kordoc", timeoutMs });
  return new Promise((resolve, reject) => {
...
const server = http.createServer(async (request, response) => {
  ...
  if ((request.method === "GET" || request.method === "HEAD") && request.url === "/api/health") {
    return sendJson(response, 200, { ok: true, adapterVersion: "ntis-2026-09-21-seoul-proxy-2", documentParser: "kordoc", agentRunner: true, openaiModel: OPENAI_MODEL, openaiSearchModel: OPENAI_SEARCH_MODEL, externalWebSearch: true, ntisConfigured: Boolean(process.env.NTIS_API_KEY) });
  }
...
module.exports = { server, compactPriorResults, promptFor, outputSchemaFor, normalizeAgentResult, reconcileFinalResult, ensureUniqueDiagnosisScores, collectExternalEvidence };
