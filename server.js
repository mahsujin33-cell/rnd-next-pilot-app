...
async function collectExternalEvidence(apiKey, agentKey, input) {
  ...
  try {
    ...
    if (uniqueSources.length < 2) {
      ...
      const retryResponse = await fetch("https://api.openai.com/v1/responses", { method: "POST", headers: { Authorization: `Bearer ${apiKey}`, "Content-Type": "application/json" }, body: JSON.stringify({ model: OPENAI_SEARCH_MODEL, tools: [{ type: "web_search", external_web_access: true, search_context_size: "medium" }], tool_choice: "required", include: ["web_search_call.action.sources"], input: retryPrompt }), signal: controller.signal });
