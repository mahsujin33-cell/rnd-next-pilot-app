...
function reconcileFinalResult(result, priorResults) {
  ...
  if (routeMismatch || technologyBlocked || industryBlocked) {
    ...
    result.primaryNext = "추가 판단 필요";
    result.confidence = Math.min(Number.isFinite(result.confidence) ? result.confidence : 0, 49);
    result.decision = `Challenger의 취약점과 Transition 추천 경로가 ${priorNext}와 일치하지 않아 최종 경로를 확정하지 않습니다.`;
...
async function handleHwpxReport(request, response) {
  try {
    ...
    response.writeHead(200, {
      ...
      "X-HWPX-Mapping-Report": encodeURIComponent(JSON.stringify(generated.mapping)),
...
module.exports = { server, compactPriorResults, promptFor, outputSchemaFor, normalizeAgentResult, reconcileFinalResult };
