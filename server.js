...
async function fetchNtisCollection(collection, dbt, searchWord, apiKey, requestId) {
  ...
  diagnosticLog("ntis.request.start", { requestId, collection, dbt, queryHash: diagnosticHash(searchWord), queryLength: searchWord.length, endpointHost: "www.ntis.go.kr", timeoutMs: NTIS_TIMEOUT_MS });
  ...
  try {
    const response = await fetch(url, { headers: { Accept: "application/xml,text/xml;q=0.9", "User-Agent": "RND-NEXT-Pilot/1.0" }, signal: controller.signal });
...
async function handleHwpxReport(request, response) {
  try {
    ...
    return sendJson(response, error.statusCode || 500, { success: false, message: String(error.message || "HWPX 보고서 생성에 실패했습니다.").slice(0, 300) });
  }
}
  ...
  return new Promise((resolve, reject) => {
    ...
    child.on("close", (code) => {
      ...
      if (code !== 0) {
        ...
        diagnosticLog("parser.process.exit", { requestId, extension, exitCode: code, stderr: diagnostic.slice(0, 300), elapsedMs: Date.now() - startedAt, error: diagnosticError(error) });
        finish(reject, error);
      } else {
        diagnosticLog("parser.process.complete", { requestId, extension, exitCode: code, outputBytes: Buffer.byteLength(output), elapsedMs: Date.now() - startedAt });
...
const server = http.createServer(async (request, response) => {
  ...
  if ((request.method === "GET" || request.method === "HEAD") && request.url === "/api/health") {
    return sendJson(response, 200, { ok: true, adapterVersion: "ntis-2026-09-21-diagnostics-1", documentParser: "kordoc", agentRunner: true, openaiModel: OPENAI_MODEL, ntisConfigured: Boolean(process.env.NTIS_API_KEY) });
  }
