...
async function handleHwpxReport(request, response) {
  try {
    ...
    const organization = cleanString(body?.organization, 120).trim();
    const generatedDate = new Date().toLocaleDateString("ko-KR").replace(/\\. /g, ". ");
    const hwpx = await markdownToHwpx(markdown, {
      gongmun: {
        preset: "개조식",
        cover: { org: organization, date: generatedDate },
        toc: false,
        pageNumbers: true,
        endMark: false,
        bodyTitleBox: false
