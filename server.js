...
const server = http.createServer(async (request, response) => {
  ...
  if ((request.method === "GET" || request.method === "HEAD") && request.url === "/api/health") {
    return sendJson(response, 200, { ok: true, adapterVersion: "ntis-2026-09-20-3", documentParser: "kordoc", agentRunner: true, openaiModel: OPENAI_MODEL, ntisConfigured: Boolean(process.env.NTIS_API_KEY) });
  }
