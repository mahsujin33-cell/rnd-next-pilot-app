(function () {
  ...
  async function parseReportWithBackend(file, token) {
    ...
    try {
      const response = await fetch("/api/documents/parse", { method: "POST", body: formData, signal: controller.signal });
      const payload = await response.json().catch(() => null);
      ...
      pendingReport.warnings = [error?.name === "AbortError"
        ? "Kordoc 문서 파싱이 120초 안에 완료되지 않았습니다. 문서 크기·구조를 확인한 뒤 다시 시도하세요."
  ...
  async function startAnalysis(event) {
    ...
    els["search-button"].innerHTML = '<i class="button-spinner"></i><span>NTIS·HWP 파싱 중…</span>';
    ...
    try {
      ...
      if (requiresParsedReport && report?.extractionStatus !== "available") {
        ...
        project.analysisError = "종료보고서 Kordoc 파싱이 완료되지 않아 분석을 시작하지 않았습니다.";
        State.save();
        renderAnalysis();
