(function () {
  ...
  async function lookupNtisAchievements(project) {
    try {
      ...
      return { results: [], counts: {}, warnings: [`NTIS 자동 조회 실패: ${String(error?.message || "연결할 수 없음").slice(0, 180)}. 성과 근거 없이 분석을 계속합니다.`], source: { provider: "NTIS", endpoint: "natRnDAllSearch", status: "network-error", queriedAt: new Date().toISOString() } };
    }
  }
  ...
  async function parseReportWithBackend(file, token) {
    ...
    try {
      ...
      if (!response.ok || !payload?.success) {
        ...
        pendingReport.warnings = Array.isArray(payload?.warnings) ? payload.warnings : [`백엔드 파싱 실패 (HTTP ${response.status})`];
      } else {
      ...
      if (token !== reportSelectionToken) return null;
      pendingReport.extractionStatus = "pending";
      pendingReport.warnings = ["문서 파싱 백엔드에 연결할 수 없습니다. node server.js로 실행하면 다시 시도할 수 있습니다."];
    }
    renderSelectedReport();
  ...
  async function runRealAnalysis(project, apiKey) {
    ...
    try {
      ...
      project.analysisError = String(error?.message || "실제 분석에 실패했습니다.").slice(0, 300);
  ...
  async function startAnalysis(event) {
    ...
    els["search-button"].innerHTML = '<i class="button-spinner"></i><span>NTIS 성과 근거 조회 중…</span>';
    ...
    navigate("analysis", project.id);
    try {
      const [report, ntisLookup] = await Promise.all([
        pendingReportRead.catch(() => pendingReport),
        lookupNtisAchievements(data)
      ]);
  ...
  function evidenceMarkup(items) {
    ...
    return `<div class="evidence-list">${evidence.map((item) => `<article class="evidence-item"><div><strong>${escapeHTML(item.claim || "확인 불가")}</strong><span class="certainty certainty-${escapeHTML(String(item.certainty || "확인 불가").replace(/\s+/g, "-"))}">${escapeHTML(item.certainty || "확인 불가")}</span></div><p>${escapeHTML(item.source || "확인 불가")} · ${escapeHTML(item.sourceType || "확인 불가")} · ${escapeHTML(item.pageOrId || "확인 불가")}</p></article>`).join("")}</div>`;
  }
  function domainDetailsMarkup(result, compact = false) {
    if (!result?.conclusion && !result?.keyFindings && !result?.evidence) return "";
    if (compact) return `<details class="agent-detail-expand"><summary>판정·근거·불확실성 펼치기</summary><div class="compact-detail"><p><b>판정</b>${escapeHTML(result.conclusion || result.summary)}</p><div><b>핵심 발견</b>${listMarkup(result.keyFindings)}</div><div><b>근거</b>${evidenceMarkup(result.evidence)}</div><div class="compact-columns"><section><b>불확실성</b>${listMarkup(result.uncertainties)}</section><section><b>병목</b>${listMarkup(result.bottlenecks)}</section></div></div></details>`;
    return `<section class="detailed-result"><div class="detail-verdict"><span>판정 · CONCLUSION</span><p>${escapeHTML(result.conclusion || result.summary)}</p></div><div class="detailed-grid"><section><h3>핵심 발견</h3>${listMarkup(result.keyFindings)}</section><section><h3>근거</h3>${evidenceMarkup(result.evidence)}</section><section><h3>불확실성</h3>${listMarkup(result.uncertainties)}</section><section><h3>핵심 병목</h3>${listMarkup(result.bottlenecks)}</section><section><h3>다음 단계 후보</h3>${listMarkup(result.nextCandidates)}</section><section><h3>추가 데이터 요청</h3>${listMarkup(result.dataRequests)}</section></div><div class="confidence-rationale"><strong>신뢰도 근거</strong><p>${escapeHTML(result.confidenceRationale || "추가 판단 필요")}</p></div></section>`;
  }
  ...
  function renderAnalysis() {
    ...
    const phaseLabel = stage === "source-loading" ? "NTIS 성과 근거 조회" : stage === "real-running" ? "실제 OpenAI 분석중" : stage === "real-failed" ? `${project.failedPhase || "OpenAI 분석"} 실패` : stage.startsWith("diagnosis") ? "1차 진단" : stage.startsWith("challenger") ? "Challenger 검증" : stage.startsWith("transition") ? "Transition 경로설계" : stage.startsWith("synthesis") ? "NEXT Synthesizer" : "분석 완료";
    ...
    const renderAgentCard = (agentIndex) => {
      ...
      const message = status === "complete" ? result.summary : status === "running" ? agent.message : waitingMessage;
    ...
    els["analysis-content"].innerHTML = `
      <header class="analysis-progress-header">
        ...
        <div class="workspace-actions"><button class="outline-button" type="button" data-export="${escapeHTML(project.id)}">↓ Markdown 보고서</button><button class="outline-button" type="button" data-open-detail="${escapeHTML(project.id)}">상세 리포트 보기</button></div>
      </header>
  ...
  function renderDetail() {
    ...
    els["detail-content"].innerHTML = `
      <header class="detail-header">
        ...
        <div class="detail-actions"><button class="outline-button" type="button" data-open-project="${escapeHTML(project.id)}">← 워크스페이스</button><button class="outline-button" type="button" data-export="${escapeHTML(project.id)}">↓ Markdown 보고서</button></div>
      </header>
  ...
  function exportReport(id) {
    ...
    const report = `# R&D NEXT 분석 보고서\n\n> ${provenance}\n\n## 분석 대상\n\n- 과제번호: ${project.projectId || "미입력"}\n- 과제명: ${project.name}\n- 수행기관: ${project.organization}\n- 연구 분야: ${project.field}\n\n## NTIS R&D 성과 참고 근거 (분석 대상 아님)\n\n- 조회 상태: ${ntisStatusText(project)}\n- 컬렉션: 논문(rpaper), 특허(rpatent), 연구보고서(rresearch)\n${achievementLines}${legacyReference}\n\n## 분석 개요\n\n- 분석 소스: ${reportSourceMode(project)}\n- NEXT 유형: ${project.nextType}\n- 검토 상태: ${project.status}\n- 보고서 생성일: ${new Date().toLocaleDateString("ko-KR")}\n\n---\n\n${attachmentSection}\n\n---\n\n${sections}\n`;
    const blob = new Blob([report], { type: "text/markdown;charset=utf-8" });
    const url = URL.createObjectURL(blob);
    const link = document.createElement("a");
    link.href = url;
    link.download = `RND_NEXT_${project.id}.md`;
