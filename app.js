(function () {
  ...
  function domainDetailsMarkup(result, compact = false) {
    ...
    const externalStatus = result.searchStatus === "success" && result.sourceCount >= 2 ? `외부근거 확보 ${result.sourceCount}개` : `외부근거 부족 · 확인된 출처 ${result.sourceCount || 0}개 / 최소 2개${result.searchError ? ` · 오류 ${result.searchError}` : ""}`;
  ...
  function critiquePanelsMarkup(result, compact = false) {
    ...
    const panels = Object.entries(labels).map(([key, label]) => {
      ...
      const compactSummary = critique.counterArguments?.[0] || critique.weakEvidence?.[0] || critique.verdict || "추가 판단 필요";
      return `<article class="critique-panel critique-${key}"><header><span>${escapeHTML(label)}</span><strong>${escapeHTML(critique.verdict || "추가 판단 필요")}</strong></header>${compact ? `<p class="critique-compact-summary">${escapeHTML(compactSummary)}</p><details><summary>상세 비판 펼치기</summary>` : ""}<div class="critique-body"><section><b>반론</b>${listMarkup(critique.counterArguments)}</section><section><b>약한 근거</b>${listMarkup(critique.weakEvidence)}</section><section><b>검증 질문</b>${listMarkup(critique.verificationQuestions)}</section><section><b>판단 반전 조건</b>${listMarkup(critique.reversalConditions)}</section></div>${compact ? "</details>" : ""}</article>`;
    }).join("");
